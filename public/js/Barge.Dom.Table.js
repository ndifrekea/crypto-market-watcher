...
})(typeof window !== undefined ? window : this, function factory(window)
{
    ...
    * @param options {{sortable : Boolean, stickyHead :Boolean, scrollableContainer : Boolean,
      * stickOnScroll : Boolean, hasCheckboxes : Boolean, lastColIsCheckCol : Boolean, manageCheckboxGroup : { onAllChecked: fn,
      *    onSomeChecked: fn,onNoneChecked: fn}}}
    * @constructor
    */
   ...
   /**
    * inserts a dataSet {@code qty} at the bottom or a specified index
    * @param data {Array<Array<*>>}
    * @param index  {Number}
    */
   Table.prototype.insert = function (data, index = null)
   {
      let numberOfRows = data.length, row = null, self = this;

      Ba.forEach(data, (rowData, i) =>
      {
         let rowDataCount = rowData.length;
         row = Bd.createEl("tr");
         Ba.forEach(rowData, function (cell, i)
         {
            row.appendChild(Bd.createEl("td",{innerHTML : cell}));
         });
   ...
   Table.prototype.getRows = function (startingRowId, endingRowId)
   {
      ...
      //return rowArray.length > 0 ? rowArray : null;
      return rowArray;
   };
   ...
   Table.prototype.applyDimensionsPrimer = function ()
   {
      let self = this,
          row1 = this.getRow(1),
          ths  = Bu.defined(row1) ? row1.children : null;

      let allTdRows = this.getRows(2),
          firstTds  = allTdRows.length > 0 ? this.getRow(2).children : null;

      let thsCount = ths.length - 1,
          widths   = [];

      //Bd.css(row1, { boxShadow : "0px 4px 3px rgba(0, 0, 0, .2)" });
      if(Bu.defined(ths && firstTds))
      {
